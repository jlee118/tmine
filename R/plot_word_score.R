...
#'
#' \code{plot_word_score} Given a vector of tf-idf weights, and the title of the document the vector belongs to,
#' produces a bar plot of the words with the highest tf-idf weights.
#' @param tf-idf_vector A numeric vector of tf-idf weights that has the same
#' number of columns as those in the abstracts DataFrame.
...
plot_word_score<- function (tfidf_vector,topn=25, paper_title) {
  ...
  title <- paste(title, "words from", sep = " ")
  title <- paste(title, paper_title, sep = " ")

  #Chose to flip the bar graph to fit as many words on the plot as possible
  ggplot2::ggplot(df, ggplot2::aes(x = reorder(words,top_words), y = top_words)) + ggplot2::geom_bar(stat="identity") +
    ggplot2::coord_flip() + ggplot2::xlab("Words") + ggplot2::ylab("Score")+ggplot2::ggtitle(title)
